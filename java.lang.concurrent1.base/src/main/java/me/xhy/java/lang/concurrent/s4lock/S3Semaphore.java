...
public class S3Semaphore extends Thread {
  private Semaphore position;
  private int id;

  public S3Semaphore(int i, Semaphore s) {
    this.id = i;
    this.position = s;
  }
  ...
  public void run() {
    try {
      ...
      // FIXME 如果这里要操作公共资源，仍然要枷锁，信号量只能做留空，不能做并发安全
  ...
  public static void main(String args[]) {
    ...
    //有十个人
    for (int i = 0; i < 10; i++) {
      list.submit(new S3Semaphore(i + 1, position));
    }
    list.shutdown();
